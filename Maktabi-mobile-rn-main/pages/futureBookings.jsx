...
import { deleteBookingsById, getDeskInfo, getUser, getUserBookings } from "../APIs/API";
import { Toast } from "react-native-toast-message/lib/src/Toast";
import { Entypo, FontAwesome, Fontisto } from "@expo/vector-icons";
import { getTokenFromLocalStorage } from "../APIs/common";


export default function FutureBookings({ navigation }) {
        ...
        try {
            await deleteBookingsById(item._id).then((res) => {
                ...
                for (let i = 0; i < table_data.length; i++) {
                    if (table_data[i][0] == moment(item.start).format('MMM DD, YYYY hh:mm A') && table_data[i][1] == moment(item.end).format('MMM DD, YYYY hh:mm A')
                        && table_data[i][2] == item.title) {
                        index = i;
                    }
                }
                if (index > -1) {
    ...
    async function getTableData() {
        ...
        await getUserBookings(user._id).then(async (res) => {
            ...
            for (let i in bookingList) {
                if (moment(bookingList[i].start).isAfter(moment())) {
    ...
    return (
        <>
            ...
            <View style={{ padding: 20, marginTop: 20, alignItems: 'center' }}>
                <Text style={{ textAlign: 'center', color: Colors.darkGreen, fontSize: 20, fontWeight: 'bold' }}>Your upcoming bookings</Text>
            </View>
            {tableData && tableData.length > 0 && <ScrollView style={styles.container}>
                <ScrollView horizontal={true}>
                    <Table borderStyle={{ borderWidth: 2, borderColor: Colors.darkGreen }}>
                        <Row data={['Start Time', 'End Time', 'End Time', 'Title', 'Actions']} widthArr={[200, 200, 200, 120]} style={styles.head} textStyle={styles.text} />
                        <Rows data={tableData} textStyle={styles.textCell} widthArr={[200, 200, 200, 120]} />
                    </Table>
                </ScrollView>
            </ScrollView>
            }
            {tableData && tableData.length == 0 && <Text style={{ textAlign: 'center', fontSize: 20, color: Colors.darkGreen }}>You have no future bookings</Text>}
